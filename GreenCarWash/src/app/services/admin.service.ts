...
x:any;
  ...
  GetSingleAddOn(id:string){
    let url = `http://localhost:8080/admin/add_ons/${id}`;
    return this.http.get(url,{responseType:'json' as 'json' }); 
  }

  GetSingleCarDetails(id:string){
    let url = `http://localhost:8080/admin/car_details/${id}`;
    return this.http.get(url,{responseType:'json' as 'json' }); 
  }

  GetSinglePromo(id:string){
    let url = `http://localhost:8080/admin/promos/${id}`;
    return this.http.get(url,{responseType:'json' as 'json' }); 
  }

  putAddOn(id:string,body:any){
    let url = `http://localhost:8080/admin/edit_addons/${id}`;
    return this.http.put(url,body,{responseType:'text' as 'json'});
  }

  putCarDetails(id:string,body:any){
    let url = `http://localhost:8080/admin/edit_cardetails/${id}`;
    return this.http.put(url,body,{responseType:'text' as 'json'});
  }

  putPromoDetails(id:string,body:any){
    let url = `http://localhost:8080/admin/edit_promo/${id}`;
    return this.http.put(url,body,{responseType:'text' as 'json'});
  }
